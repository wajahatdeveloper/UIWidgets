...
namespace UIWidgets
{
    ...
    public class Stepper : UIBehaviour
    {
...
#if UNITY_EDITOR
        [ContextMenu("Refresh Stepper Layout")]
        private void RefreshStepperLayout()
        {
        ...
        private Selectable[] GetSides()
        {
            ...
            if (buttons.Length != 2)
            {
                DebugX.Logger(LogChannels.UI).Error("[UI:ERROR:Stepper] A stepper must have two Button children (found {Count}).", buttons.Length);
