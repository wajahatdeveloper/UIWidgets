...
namespace AetherNexus.UIWidgets
{
    ...
    public class RadialSlider : MonoBehaviour, IPointerEnterHandler, IPointerDownHandler, IPointerUpHandler, IDragHandler
    {
        ...
        private void Update()
        {
            if (!isPointerDown && !lerpInProgress)
            {
                return;
            }
            ...
            if (lerpInProgress)
            {
                if (!Mathf.Approximately(Value, m_lerpTargetAngle))
                {
                ...
                if (m_currentLerpTime >= m_lerpTime || Mathf.Approximately(Value, m_lerpTargetAngle))
                {
        ...
        {
            _onValueChanged.Invoke((int)(Value * 360f));
            _onTextValueChanged.Invoke(((int)(Value * 360f)).ToString());
        }
