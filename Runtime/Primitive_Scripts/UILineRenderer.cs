...
namespace AetherNexus.UIWidgets
{
    ...
    public class UILineRenderer : UIPrimitiveBase
	{
		...
		// Reused across mesh rebuilds to avoid per-rebuild allocation; cleared at the start of each PopulateMesh.
		private readonly List<UIVertex[]> m_segmentsCache = new List<UIVertex[]>();
		...
		if (vh.currentVertCount > 64000) {
			DebugX.Logger(LogChannels.UI).Error("[UI:ERROR] UILineRenderer vertex count [{VertexCount}] exceeds 64000. Cannot draw.", vh.currentVertCount);
			vh.Clear ();
			return;
		}
		...
		DebugX.Logger(LogChannels.UI).Error("[UI:ERROR] CreateLineCap: SegmentType must be Start or End.");
		return null;
		}
		...
		if (rectTransform.position != Vector3.zero)
		{
			DebugX.Logger(LogChannels.UI).Warning("[UI:WARN] UILineRenderer: use on RectTransform at (0,0,0). Do not use in child objects.");
		}
        }
    }
}
