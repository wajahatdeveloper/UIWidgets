...
namespace AetherNexus.UIWidgets
{
    ...
    public class SafeArea : MonoBehaviour
    {
		...
		if (Panel == null)
		{
			DebugX.Builder(LogChannels.UI).WithContext(gameObject).Error("Cannot apply safe area - no RectTransform found on {Name}", name);
			Destroy (gameObject);
		}
        ...
        // Safe area only changes on rotation/resize, so polling every frame is wasteful.
        // Throttle the check to every N frames instead of every Update.
        const int RefreshFrameInterval = 10;
        int frameCounter = 0;

        void Update ()
        {
            if (++frameCounter < RefreshFrameInterval)
            {
                return;
            }
            frameCounter = 0;
		...
		if (Logging)
		{
			DebugX.Logger(LogChannels.UI).Info("[UI:INFO] Safe area applied to {Name}: x={X} y={Y} w={Width} h={Height}",
