// Soft alpha masking for UGUI graphics. A mask texture is stretched over a rect (set from
// UISoftMask.cs via _WorldToMask/_MaskRect) and its alpha fades the graphic. Structured like
// UI/Default so stencil masks and RectMask2D keep working.
Shader "UIWidgets/SoftMask"
{
    Properties
    {
        ...
        _SoftMaskTex ("Soft Mask (alpha)", 2D) = "white" {}
        _SoftMaskCutoff ("Cutoff", Range(0,1)) = 0
        _SoftMaskInvert ("Invert", Float) = 0
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            fixed4 frag(v2f IN) : SV_Target
            {
                ...
                half mask = tex2D(_SoftMaskTex, saturate(IN.maskUV)).a;
