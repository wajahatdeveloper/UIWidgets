...
namespace UIWidgets
{
    ...
    public class CardStack2D : MonoBehaviour
    {
        ...
        void Start()
        {
            ...
            ///Cache the CanvasGroup of each card so Update doesn't call GetComponent every frame.
            cardCanvasGroups = new CanvasGroup[cards.Length];
            for (int i = 0; i < cards.Length; i++)
            {
                cardCanvasGroups[i] = cards[i].gameObject.GetComponent<CanvasGroup>();
            }
        ...
        void Update()
        {
            if (canUseHorizontalAxis)
            {
                ...
                if ((UIInput.GetAxisRaw("Horizontal") < 0 || UIInput.GetKey(leftButton)) && cardArrayOffset > 0)
                {
                    cardArrayOffset--;
                    StartCoroutine(ButtonCooldown());
                }
                else if ((UIInput.GetAxisRaw("Horizontal") > 0 || UIInput.GetKey(rightButton)) && cardArrayOffset < cards.Length - 1)
                {
                    cardArrayOffset++;
                    StartCoroutine(ButtonCooldown());
                }
            ...
            for (int i = 0; i < cards.Length; i++)
            {
                ...
                if (Mathf.Abs(cards[i].localPosition.x - cardPositions[i + cardArrayOffset].x) < 0.01f)
                {
                    ...
                    {
                        cardCanvasGroups[i].interactable = false;
                    }
                }
            }
        }
